			...
			.extend(
					$.validator,
					{
						...
						messages : {
							...
							mac:"请输入正确的mac地址",
							phone:"请输入正确格式的电话号码",
							usernameFormat:"不能包含汉字和非法字符（-和_除外）但不能以.或-或_开头结尾",
							nameFormat:"请不要包含非法字符",
							versionFormat:"请不要包含汉子和非法字符",
							mask:"请输入正确格式的掩码",
								...
								$(this.currentForm)
										.validateDelegate(
												":text, [type='password'], [type='file'], select, textarea, "
														...
														+ "[type='range'], [type='color'],[type='ip'],[type='mac'],[type='mask'],"
														+ "[type='phone'], [type='nameFormat'],[type='versionFormat'],[type='usernameFormat']",
						...
						methods : {
							...
							usernameFormat:function(value,element){
								//不能包含汉字和非法字符
								/*
								var result = true;
								if(pattern.test(value)){
								...
								//var pattern =/[@#\$%\^&\*]|[\u4e00-\u9fa5]|[^\x00-\xff]+/g ;
								var result = pattern.test(value);
								if(this.optional(element)){
									return true;
								}
								var a = value.indexOf('-');
								var b = value.indexOf('_');
								var c = value.lastIndexOf('-');
								var d = value.lastIndexOf('_');
								
								var e = value.indexOf('.');
								var f = value.lastIndexOf('.');
								
								if(result){
									if(a==0 || b==0 || e==0 || c==value.length-1 || d==value.length-1 || f==value.length-1){
										return false;
									}
								}
								return result;
							},
							nameFormat:function(value,element){
								//不能包含非法字符
								...
								var d = value.lastIndexOf('_');
								if(result){
									if(a==0 || b==0 || c==value.length-1 || d==value.length-1){
										return false;
									}
								}
								return result;
							},
							versionFormat:function(value,element){
								//可以包含字母和数字和英文输入法.号
							...
							ip:function(value,element){
								...
								var ipv6flag = false;
								 ...
								 ipv4flag = this.optional(element)||/^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/.test(value);								
								 return (ipv4flag||ipv6flag);
							},
