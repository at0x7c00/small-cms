	  			...
	  			<div class="module-group">
		  			<div class="module left lg" style="min-height:520px;width:1250px;">
		  				<div class="module-title">
		  					搜索"<n:shorthand length="10" content="${key }"></n:shorthand>"结果
		  				</div>
		  				...
		  				<ul class="module-content">
							...
							<c:forEach items="${page.list}" var="chapter" varStatus="s">
								<li>
									...
									[${chapter.id }]<n:shorthand length="50" content="${chapter.title}"></n:shorthand> 
									</a>
									<div>
										...
										<div class="search-abstract">
											...
											<div class="sub-abstract">
												<span>${chapter.source eq 'chapter' ? '文章':'质量档案' }</span> 
							...
							<div class="page-bar">
								<c:choose>
									<c:when test="${page.pageNum>1}">
										<a href="${basePath}search.do?key=${key}&pageNum=${page.pageNum-1}&${params }#content" style="padding-left:15px;padding-right:15px;"><i class="fa fa-angle-left"></i></a>
									</c:when>
									<c:otherwise>
								...
								<c:forEach items="${page.pageBarIndex}" var="x"> 
									<a href="${basePath}search.do?key=${key}&pageNum=${x}&${params }#content" class="${x eq page.pageNum ? 'active':'' }">${x}</a>
								</c:forEach>
								
								<c:choose>
									<c:when test="${page.pageNum<page.countPage}">
										<a href="${basePath}search.do?key=${key}&pageNum=${page.pageNum+1}&${params }#content" style="padding-left:15px;padding-right:15px;"><i class="fa fa-angle-right"></i></a>
									</c:when>
									<c:otherwise>
