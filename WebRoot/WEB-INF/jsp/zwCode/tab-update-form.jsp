								...
								<label class="input">
										<input name="${propName}[${trIndex}].companyName"
											...
											class="textInput required"
											 value="<c:out value="${tmpTmpBean.companyName}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
										...
										<select id="${propName}[${trIndex}].code.id" name="${propName}[${trIndex}].code.id" class=" comboxed">
											...
											<c:forEach items="${commonFileList}" var="commonFile" >
												<option value="${commonFile.id}" ${tmpTmpBean.code.id eq commonFile.id ? 'selected' : ''}><c:out value="${commonFile.manageKey}"/></option>
											</c:forEach>
										</select><i></i>
										</label>
				</td>