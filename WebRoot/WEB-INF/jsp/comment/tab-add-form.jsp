								...
								<label class="input">
										<input name="${propName}[${trIndex}].user"
											...
											class="textInput required"
											 value="<c:out value="${tempBean.user}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
				...
				<td>
								<label class="input">
										<input name="${propName}[${trIndex}].phone"
											...
											class="textInput required"
											 value="<c:out value="${tempBean.phone}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
				...
				<td>
								...
								<option value=""><spring:message code="base.common.selectone"/></option>
									<c:forEach items="${yesNoMap}" var="entry" >
										<option value="${entry.key}" ${tempBean.replyStatus eq entry.key ? 'selected' : ''}><c:out value="${entry.value}"/></option>
									</c:forEach>
								</select><i></i>
								</label>
				</td>